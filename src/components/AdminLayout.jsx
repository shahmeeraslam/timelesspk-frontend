...
import { 
  ...
  RiCloseLine, RiUserLine, RiVerifiedBadgeLine
...
const AdminLayout = () => {
  ...
  useEffect(() => {
    ...
    // Listen for storage changes (helpful if profile is updated in another tab/component)
    window.addEventListener('storage', syncUserData);
    return () => window.removeEventListener('storage', syncUserData);
  }, [navigate, location.pathname]); // Re-sync on route change to catch local updates
  ...
  return (
    <div 
      ...
      className="min-h-screen flex overflow-x-hidden font-sans"
    >
      ...
      >
        <div className="flex flex-col h-full p-8">
          <div className="flex items-center justify-between mb-16">
            ...
            <button className="lg:hidden p-2" onClick={() => setIsMobileMenu(false)}>
              <RiCloseLine size={24} />
            </button>
          </div>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link 
                  ...
                  className={`flex items-center gap-4 px-5 py-4 text-[10px] uppercase tracking-[0.2em] transition-all group ${isActive ? 'font-black' : 'hover:text-[var(--brand-main)]'}`}
                >
          ...
          <div className="mt-auto pt-8 border-t" style={{ borderColor: 'var(--brand-border)' }}>
            <button 
              ...
              className="flex items-center gap-4 px-2 py-3 text-[10px] uppercase tracking-[0.2em] hover:text-red-500 transition-colors w-full"
            >
              <RiLogoutBoxRLine size={18} /> Exit_Protocol
            </button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 lg:ml-[280px] flex flex-col min-w-0">
        <header 
          ...
          className="h-24 border-b flex items-center justify-between px-8 sticky top-0 z-40 bg-opacity-90 backdrop-blur-xl"
        >
          ...
          <div className="flex items-center gap-6 ml-auto">
            ...
            <div className="text-right hidden sm:block">
              <div className="flex items-center justify-end gap-2">
                ...
                {user?.isVerified && <RiVerifiedBadgeLine size={14} style={{ color: 'var(--brand-accent)' }} />}
              </div>
            ...
            <div className="relative group">
              <div 
                className="w-12 h-12 border-2 p-0.5 transition-transform group-hover:rotate-3" 
                style={{ borderColor: 'var(--brand-border)', backgroundColor: 'var(--brand-alt)' }}
              >
