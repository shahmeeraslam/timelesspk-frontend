...
// Added 'data' prop which comes from your new CMS fetch
const Hero = ({ scrollProgress, data }) => {
  const navigate = useNavigate();

  // Fallback content in case CMS data isn't loaded yet
  const content = {
    titleTop: data?.hero?.titleTop || "Infinite",
    titleBottom: data?.hero?.titleBottom || "Potential",
    subtitle: data?.hero?.subtitle || "Crafting the artifacts of a future defined by purpose and elegance.",
    bgImage: data?.hero?.bgImage || "https://images.unsplash.com/photo-1490481651871-ab68de25d43d?q=80&w=2070",
    tagline: data?.hero?.tagline || "Horizon_Found_2026"
  };
  ...
  return (
    <section className="relative h-screen flex flex-col justify-center px-6 md:px-20 overflow-hidden">
        ...
        <div className="lg:col-span-4 flex flex-col gap-10 lg:gap-12 lg:border-l-2 border-[var(--brand-main)] lg:pl-10 text-center lg:text-left">
          <div className="space-y-6">
            ...
            <div className="font-mono text-[9px] space-y-2 opacity-60 uppercase tracking-[0.2em] max-w-[250px] mx-auto lg:mx-0">
              <p className="flex justify-between border-b border-white/10 pb-1 lg:border-0"><span>Status:</span> <span>Active_Sync</span></p>
              <p className="flex justify-between border-b border-white/10 pb-1 lg:border-0"><span>Node:</span> <span>Archive_HQ</span></p>
            </div>
          </div>
      ...
      {/* Global CSS for animations remained the same */}
      <style>{`
