  ...
  return (
    <div className="fixed inset-0 z-[500] flex justify-end print:hidden">
      ...
      <motion.div 
        ...
        exit={{ x: '100%' }} 
        className="relative w-full max-w-2xl bg-[#080808] border-l border-white/10 h-full p-8 md:p-16 overflow-y-auto shadow-[0_0_100px_rgba(0,0,0,1)]"
      >
        {/* Header Actions */}
        ...
        <div className="space-y-20">
          ...
          <div className="space-y-4">
            ...
            <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest">
              Entry_Created: {new Date(order.createdAt).toLocaleString()}
            </p>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              {order.items?.map((item, idx) => {
                const pData = item.productId || item;
                return (
                  <div key={idx} className="flex gap-8 p-6 bg-white/[0.02] border border-white/5 items-center group/item">
                    <div className="w-16 h-20 bg-zinc-900 overflow-hidden grayscale group-hover/item:grayscale-0 transition-all border border-white/5">
                      <img src={pData.image?.[0]?.url || item.img} className="w-full h-full object-cover" alt="" />
                    </div>
                    <div className="flex-grow space-y-3">
                      <h4 className="text-[11px] font-bold uppercase tracking-[0.2em]">{pData.name}</h4>
                      <div className="flex flex-wrap gap-6 items-center">
                        ...
                        <div className="flex gap-4 text-[9px] font-mono uppercase text-white/40 tracking-widest">
                          ...
                          <span className="text-white/20">{(pData.price || 0).toLocaleString()} PKR</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 py-12 border-y border-white/5">
            ...
            <div className="space-y-6">
              ...
              <p className="text-[11px] font-mono leading-relaxed text-white/60 uppercase">
                ...
                Pakistan_{order.address?.zipcode}
              </p>
            </div>
          </div>

          {/* Financial Summary */}
          <div className="bg-white text-black p-10 flex flex-col gap-2 shadow-2xl">
            ...
            <span className="text-5xl font-serif italic tracking-tighter">
              {(order.amount || 0).toLocaleString()} PKR
            </span>
          </div>
        </div>
      </motion.div>
